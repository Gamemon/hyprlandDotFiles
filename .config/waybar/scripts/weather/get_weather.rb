...
module Config
  @settings = {
    ...
    snapshot_number_of_days: 3, # max 3, used in week view
    latitude: 41, # or float
    longitude: -91, # or float
    refresh_interval: 180, # seconds between API calls
...
module CacheManager
  class << self
    ...
    def fresh?(refresh_interval = 120)
    ...
    def resolve_location(settings)
      if settings[:latitude].to_s == 'auto' || settings[:longitude].to_s == 'auto'
      ...
      else
        {
          ...
          location_name: 'Iowa City, IA, USA (Non Auto)'
        }
      end
    end
...
private def run_weather_update(force_refresh: false)
  ...
  refresh_interval = settings[:refresh_interval] || 120
